...
// Define the barcode module width
#define BARCODE_MODULE_WIDTH 1
...
// Calculate the barcode module width
int module_width = ceil(width / strlen(data));
...
for ( i = 0; i < strlen(data); i++) {
	...
	int barcode_patterns[100][100];
	const unsigned char *bar_pattern = barcode_patterns[symbology][char_code - BARCODE_CHAR_A];
	
	// Draw the barcode bar
	int j, k;
	for ( j = 0; j < module_width; j++) {
	  for ( k = 0; k < height; k++) {
	    int pixel_index = (current_x + j * BARCODE_MODULE_WIDTH) * 3 + k * 3 * width;
	    barcode[pixel_index] = bar_pattern[j % BARCODE_BAR_WIDTH];
	    barcode[pixel_index + 1] = bar_pattern[j % BARCODE_BAR_WIDTH];
	    barcode[pixel_index + 2] = bar_pattern[j % BARCODE_BAR_WIDTH];
	  }
	}
